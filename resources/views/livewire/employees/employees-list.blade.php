    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Servicios</th>
                            ...
                            <tr wire:key="employee-{{ $employee->id }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $employee->nombre }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @if($employee->telefono)
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $employee->telefono }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-gray-100">{{ $employee->services->count() }} servicio(s)</span>
                                </td>
                ...
                <div class="flex items-center justify-center min-h-screen px-4">
                    ...
                    <div class="relative bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full p-6">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Detalles del Empleado</h3>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-gray-900 dark:text-gray-100">{{ $selectedEmployee->nombre }}</p>
                            </div>

                            <div>
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                                <p class="text-gray-900 dark:text-gray-100">{{ $selectedEmployee->email }}</p>
                            </div>

                            @if($selectedEmployee->telefono)
                                <div>
                                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Teléfono</label>
                                    <p class="text-gray-900 dark:text-gray-100">{{ $selectedEmployee->telefono }}</p>
                                </div>
                            @endif

                            @if($selectedEmployee->services->count() > 0)
                                <div>
                                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Servicios asignados</label>
                                    <ul class="mt-2 space-y-2">
                                        @foreach($selectedEmployee->services as $service)
                                            <li class="flex justify-between items-center text-gray-900 dark:text-gray-100">
                                                <span>{{ $service->nombre }}</span>
                                                <span class="text-sm text-gray-500">${{ number_format($service->precio, 2) }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                            @else
                                <div>
                                    <p class="text-gray-500 dark:text-gray-400 italic">No tiene servicios asignados</p>
                                </div>
                            @endif
                        </div>
        ...
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full">
                ...
                <p class="text-sm text-gray-500 mb-6">¿Estás seguro de que deseas eliminar este empleado? Esta acción no se puede deshacer.</p>
